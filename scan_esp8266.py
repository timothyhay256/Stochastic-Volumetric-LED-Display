import json # TODO: Android ip camera
import cv2, queue, threading, time
from receive_esp8266.esp8266_udp import set_color # This function blocks until the color has been properly set.
from simpleLog import log
import time

numLed = 200 #num of leds
ledPos=[[0]*3]*(numLed+1)
...
vid = VideoCapture(2) # Camera index 2 for external camera.
...
cv2.setWindowTitle(winname, 'Drag the mouse around the jar.')
...
gray = cv2.GaussianBlur(gray, (41, 41), 0)
(minVal, maxVal, minLoc, maxLoc) = cv2.minMaxLoc(gray)
log("darkest for second LED (obstructed) is:"+str(maxVal))
darkest = maxVal
image = orig.copy()
cv2.circle(image, maxLoc, 20, (255, 0, 0), 2)
...
cv2.setWindowTitle(winname, 'The brightest non-LED spot should be circled')
cv2.imshow(winname, image)
set_color(1, 0, 0, 0)
cv2.waitKey(0)
